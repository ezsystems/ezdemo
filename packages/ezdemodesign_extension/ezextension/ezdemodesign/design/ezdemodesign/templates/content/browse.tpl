...
{else}
    ...
    <h1 class="long">{'Browse'|i18n( 'design/ezdemodesign/content/browse' )} - {$main_node.name|wash}</h1>
    </div>

    <p>{'To select objects, choose the appropriate radiobutton or checkbox(es), and click the "Select" button.'|i18n( 'design/ezdemodesign/content/browse' )}</p>
    <p>{'To select an object that is a child of one of the displayed objects, click the parent object name to display a list of its children.'|i18n( 'design/ezdemodesign/content/browse' )}</p>
{/if}

{def $current_node=fetch( content, node, hash( node_id, $browse.start_node ) )}
{if $browse.start_node|gt( 1 )}
    <h2 class="context-title">
    <a href={concat( '/content/browse/', $main_node.parent_node_id, '/' )|ezurl}><img src={'back-button-16x16.gif'|ezimage} alt="{'Back'|i18n( 'design/ezdemodesign/content/browse' )}" /></a>
    {$current_node.name|wash}&nbsp;[{$current_node.children_count}]</h2>
{else}
    <h2 class="context-title"><img src={'back-button-16x16.gif'|ezimage} alt="Back" />&nbsp;{'Top level'|i18n( 'design/ezdemodesign/content/browse' )}&nbsp;[{$current_node.children_count}]</h2>
{/if}
...
{/if}
        ...
        <input class="button" type="submit" name="SelectButton" value="{'Select'|i18n('design/ezdemodesign/content/browse')}" />
